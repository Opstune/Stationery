    ...
    if request.method == "POST":
        ...
        if request.method == "POST":
            if "cancel_order" in request.form:  # กรณียกเลิกคำสั่งซื้อ
                ...
                # ดึงข้อมูลจาก order_check
                order_status = order_check[0][3]  # สถานะคำสั่งซื้อ
                customer_id = order_check[0][1]  # รหัสลูกค้า
                stock_id = order_check[0][2]  # รหัสสินค้า

                if order_status != 'ยกเลิก':  # Only if the order is not canceled yet
                    # ✅ Update the order status to 'ยกเลิก' (canceled)
                    ...
                    print(f"Updating Order ID {order_id_to_cancel} to 'ยกเลิก'")  # Debugging print statement

                    # ดึงข้อมูลลูกค้า
                    customer_result = execute_data(f"SELECT * FROM tbCustomer WHERE CustCode = '{customer_id}'")
                    if customer_result:
                        print("Customer Data:", customer_result)
                    else:
                    ...
                    # ดึงชื่อสินค้า
                    stock_result = execute_data(f"SELECT * FROM tbStock WHERE StockCode = '{stock_id}'")
                    if stock_result:
                        product_name = stock_result[0][4]  # ชื่อสินค้า
                    else:
                        product_name = "ไม่มีสินค้า"
